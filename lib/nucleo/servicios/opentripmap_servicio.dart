...
class OpenTripMapServicio {
  static Future<List<dynamic>> buscarLugaresCulturales(
    double lat,
    double lng, {
    int radius = 1000,
  }) async {
    ...
    final url = Uri.parse(
      'https://api.opentripmap.com/0.1/en/places/radius?radius=5000&lon=$lng&lat=$lat&kinds=interesting_places&apikey=$apiKey',
    );
  ...
  static Future<Map<String, double>?> obtenerCoordenadas(String destino) async {
    final apiKey = dotenv.env['OPENTRIPMAP_API_KEY']?.trim();

    final url = Uri.parse(
      'https://api.opentripmap.com/0.1/en/places/geoname?name=$destino&apikey=$apiKey',
    );
